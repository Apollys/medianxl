   ...
   while (not done):
      ...
      for i, coords in enumerate(median.inv_iter(*shrine_size)):
         ...
         for charge in range(10):
            ...
            # Get text
            item_stats = median.get_item_text()
            dex = item_parser.get_attr(item_stats, "Dexterity")
            life = item_parser.get_life(item_stats)
            frw = item_parser.get_frw(item_stats)
            pdex = item_parser.get_pattr(item_stats, "Dexterity")
            #if (avg_flat_dmg > best_avg_flat_dmg):
            #   best_avg_flat_dmg = avg_flat_dmg
            #   corr_enh_dmg = enh_dmg
            # log_string = '=========================\n\n' + item_stats + '\n'
            # util.log(log_string, 'item_log.txt')
            if ( (dex >= DEX_THRESH) and (pdex >= PDEX_THRESH) ):
               print("Dex:{0}  Life:{1}   frw:{2}".format(dex, life, frw))
            sleep_ms(100)
            # Check if we are done
            if ( (dex >= DEX_THRESH) and (life >= LIFE_THRESH) and (pdex >= PDEX_THRESH) and (frw >= FRW_THRESH) ):
               done = True
               break
         # End of shrine charges loop
         # For testing, break early
